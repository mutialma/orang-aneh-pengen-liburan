...
export async function POST(req: Request) {
  ...
  try {
    ...
    // PERBAIKAN TOTAL FORMAT TANGGAL (MURNI LOKAL STRING TANPA UTC BENCIAN API)
    const targetCheckin = new Date();
    targetCheckin.setDate(targetCheckin.getDate() + 10); // set 10 hari ke depan agar aman dari sisa kamar kosong
    ...
    try {
      for (let i = 0; i < shuffledCities.length; i++) {
        ...
        if (destId) {
          ...
          // PERBAIKAN FINAL: Gunakan URLSearchParams agar semua mandatory parameter terisi dan ter-encode sempurna
          const queryParams = new URLSearchParams({
            ...
            adults_number: "1",
            room_number: "1",
            order_by: "popularity",       // INI WAJIB (Sering jadi penyebab 422 kalau hilang)
            units: "metric",              // INI WAJIB 
            locale: "en-gb",              // INI WAJIB (Pakai en-gb standar Booking.com)
    ...
    // =========================================================================
    // STEP 2: CEK HASIL API (MODE STRICT - TANPA DUMMY)
    // =========================================================================
