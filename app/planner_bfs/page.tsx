...
export default function PlannerPage() {
  ...
  // ── Result state ─────────────────────────────────────────────────────────
  // Menggunakan 'any' atau disesuaikan dengan interface backend yang baru
  ...
  const rawItinerary = results?.main?.itinerary
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-emerald-50">
      ...
      <section className="py-12 px-4 text-center">
        ...
        <p className="text-gray-500 max-w-lg mx-auto text-sm leading-relaxed">
          Masukkan budget, durasi, dan preferensimu. AI kami akan menghitung rute paling efisien dan hemat.
        </p>
      </section>
        ...
        <div className="max-w-3xl mx-auto bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="bg-sky-50 rounded-2xl p-4 border border-sky-100">
              <p className="text-xs font-bold text-sky-700 mb-3"> Aktif: BFS Ganti algoritma?</p>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { href: "/planner",     label: "Beam Search",  },
                  { href: "/planner_ucs", label: "UCS",          },
                  { href: "/planner_bfs", label: "BFS",          active: true },
                    ].map(algo => (
                  <a
                    key={algo.href}
                    href={algo.href}
            ...
            >
              {loading
                ? <><Loader2 size={18} className="animate-spin" /> Menghitung rute paling hemat...</>
            ...
            <div className="max-w-3xl mx-auto">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="font-display font-black text-xl text-gray-900">
                  Rekomendasi Termurah & Terbaik Untukmu
                </h2>
              </div>
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex gap-1 flex-wrap justify-end mt-2">
                        {results.main.badges?.map((b: string) => <Badge key={b} label={b} />)}
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      ...
                      <div key={c.label} className="bg-gray-50 rounded-2xl p-3 text-center">
                        ...
                        <div className="font-black text-sm text-gray-800">{fmt(c.value || 0)}</div>
                  ...
                  {itineraryOpen && (
                    <div className="mt-4 slide-in">
                      ...
                      <div className="space-y-3">
                        {(itineraryDays[activeDay] || itineraryDays[0] || []).map((item: any, idx: number) => (
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-3xl p-5 border border-amber-100">
                  ...
                  <p className="text-gray-600 text-xs leading-relaxed">
                    {upgradeTips.find(t => budget >= t.from && budget <= t.to)?.message
                      ?? "Tambah Rp300.000 bisa dapat hotel lebih nyaman dan lebih dekat ke pusat."}
                  </p>
                </div>
                ...
                <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-3xl p-5 border border-purple-100">
                  ...
                  <ul className="space-y-2">
                    {results.main.hiddenGems?.map((g: string, i: number) => (
                      <li key={i} className="flex items-start gap-1.5 text-xs text-gray-600">
